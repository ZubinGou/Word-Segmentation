...
def simple_run():
    ...
    word_train, word_dev, label_train, label_dev = dev_split(
        config.train_dir)  # 参数为训练集
    # simple run without k-fold
    run(word_train, label_train, word_dev, label_dev, vocab,
        device)  # 运行训练集和测试集


def run(word_train, label_train, word_dev, label_dev, vocab, device):
    """
    函数功能： 
    ...
    # 测试集   验证集  词表 设备
    # build dataset
    train_dataset = SegDataset(word_train, label_train, vocab,
                               config.label2id)  # 训练集，包含了字列表和标签列表，这两个列表一一对应
    dev_dataset = SegDataset(word_dev, label_dev, vocab,
                             config.label2id)  # 验证集
    # build data_loader
    train_loader = DataLoader(
        train_dataset,
        batch_size=config.batch_size,  # 创造一个迭代器，方便接下来的模型迭代的访问训练集
        shuffle=True,
        collate_fn=train_dataset.collate_fn)
    dev_loader = DataLoader(
        dev_dataset,
        batch_size=config.batch_size,  # 创造一个迭代器，方便接下来的模型迭代的访问验证集
        shuffle=True,
        collate_fn=dev_dataset.collate_fn)
    # model
    model = BiLSTM_CRF(
        embedding_size=config.embedding_size,  # 初始化模型
    ...
    optimizer = optim.Adam(
        ...
        betas=config.betas)  #optimizer是一个优化器，可以保存当前的参数，并根据计算得到的梯度来更新参数
    ...
    # train
    train(train_loader, dev_loader, vocab, model, optimizer, scheduler,
          device)  # 模型训练、验证
    # test
    with torch.no_grad():
