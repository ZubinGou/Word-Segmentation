...
def train(train_loader, dev_loader, vocab, model, optimizer, scheduler, device):
    ...
    for epoch in range(1, config.epoch_num + 1):        # 不停的进行模型训练
        epoch_train(train_loader, model, optimizer, scheduler, device, epoch)  #训练模型参数
        # 模型参数验证
        with torch.no_grad():
...
def test(dataset_dir, vocab, device):
    ...
    logging.info("final test loss: {:.2f}, f1 score: {:.2f}, precision:{:.2f}, recall: {:.2f}"
                     .format(test_loss, f1 * 100, p * 100, r * 100))
    return test_loss, f1
