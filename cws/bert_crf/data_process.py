...
class Processor:
    ...
    def get_examples(self, mode):
        ...
        """
        input_dir = self.data_dir + str(mode) + '.utf8'
        ...
        with open(input_dir, 'r', encoding='utf-8') as f:
            ...
            for line in f:
                ...
                if len(words) > config.max_len:
                    # 直接按最大长度切分
                    sub_word_list = get_sub_list(words, config.max_len - 5,
                                                 config.sep_word)
                    sub_label_list = get_sub_list(labels, config.max_len - 5,
                                                  config.sep_label)
                ...
                assert len(word_list) == len(
                    label_list), "label 句子数量与 word 句子数量不匹配"
            print("We have", num, "lines in", mode, "file processed")
            print("We have", sep_num, "lines in", mode,
                  "file get sep processed")
            # 保存成二进制文件
...
def read_file(mode='training'):
    """读取文件"""
    input_dir = config.data_dir + str(mode) + '.utf8'
    word_list = []
    len_list = []
    with open(input_dir, 'r', encoding='utf-8') as f:
        for line in f:
...
def get_len(mode='training'):
    ...
    lens = {'<100': 0, '100-400': 0, '400-500': 0, '500-1000': 0, '>1000': 0}
    ...
    for i in len_list:
        if i <= 100:
            ...
            lens['400-500'] += 1  # 94 sentences' len > 256 in test.txt
        elif 500 < i <= 1000:
            lens['500-1000'] += 1
        elif i > 1000:
            lens['>1000'] += 1
